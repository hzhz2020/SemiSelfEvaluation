...
from libml.Echo_data import Echo as dataset
...
def main(args):
    ...
    TMED2_DEV56_dataseed1_precalculated_class_weights = [0.137, 0.398, 0.192, 0.273]
    weights = torch.Tensor(TMED2_DEV56_dataseed1_precalculated_class_weights)
...
#     TissueMNIST_mean = (0.0988, 0.0988, 0.0988)
#     TissueMNIST_std = (0.0785, 0.0785, 0.0785)
#     PathMNIST_mean = (0.7403, 0.5310, 0.7062)
#     PathMNIST_std = (0.0710, 0.1018, 0.0719)
    TMED2_DEV56_dataseed1_mean = (0.0636, 0.0636, 0.0636)
    TMED2_DEV56_dataseed1_std = (0.1426, 0.1426, 0.1426)
    ...
    transform_labeledtrain = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=TMED2_DEV56_dataseed1_mean, std=TMED2_DEV56_dataseed1_std)
    ])
    ...
    l_train_dataset = dataset(args.l_train_dataset_path, transform_fn=TransformTwice(transform_labeledtrain))
...
#     global_best_train_ema_acc_at_val = 0
    ...
    while total_used_time <= args.total_hour: #run for 100 hour
        ...
        experiment_name = "Optimizer-{}/LrSchedule-{}_LrCycleEpochs-{}_LrWarmupEpochs-{}/lr-{}_wd-{}_alpha-{}".format(args.optimizer_type, args.lr_schedule_type, args.lr_cycle_epochs, args.lr_warmup_epochs, args.lr, args.wd, args.alpha)
...
#                 best_train_raw_acc_at_val = 0
    ...
    #         train_predictions_save_dict = dict()
            ...
            #train
            train_labeled_loss_list = train_one_epoch(args, weights, l_loader, model, ema_model, optimizer, scheduler, epoch)
...
#             save_pickle(os.path.join(args.experiment_dir, 'losses'), 'losses_dict.pkl', train_loss_dict)
            ...
            if epoch % args.eval_every_Xepoch == 0:
                #val
                val_loss, val_raw_acc, val_ema_acc, val_true_labels, val_raw_predictions, val_ema_predictions = eval_model(args, val_loader, model, ema_model.ema, epoch, evaluation_criterion='balanced_accuracy')
...
#                 val_predictions_save_dict['ema_predictions'] = val_ema_predictions
                ...
                #test
                test_loss, test_raw_acc, test_ema_acc, test_true_labels, test_raw_predictions, test_ema_predictions = eval_model(args, test_loader, model, ema_model.ema, epoch, evaluation_criterion='balanced_accuracy')
...
#                 test_predictions_save_dict['ema_predictions'] = test_ema_predictions
    ...
    #                 save_pickle(os.path.join(args.experiment_dir, 'best_predictions_at_raw_val'), 'train_predictions.pkl', train_predictions_save_dict)
                ...
                if best_val_raw_acc > global_best_val_raw_acc:
                    ...
                    global_best_hypercombo_raw['alpha']=alpha
    ...
    #                 save_pickle(os.path.join(args.experiment_dir, 'best_predictions_at_ema_val'), 'train_predictions.pkl', train_predictions_save_dict)
                ...
                if best_val_ema_acc > global_best_val_ema_acc:
                    ...
                    global_best_hypercombo_ema['alpha']=alpha
                ...
                logger.info('EMA Best, validation/test/train %.2f %.2f' % (best_val_ema_acc, best_test_ema_acc_at_val))
...
if __name__ == '__main__':
    ...
    args.nimg_per_epoch = 355160 #total size of labeled + unlabeled set for TMED2
